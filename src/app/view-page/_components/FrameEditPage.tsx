  ...
  return (
    <div className="max-w-[632px]">
      <div className="bg-white border border-[#E4E4E7] rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-4">
            <div className="relative w-16 h-16 rounded-full overflow-hidden border border-gray-300">
              {image ? <CldImage width="64" height="64" src={image} alt="Profile" className="w-full h-full object-cover" /> : <img src="Profile.png" className="w-full h-full object-cover" alt="Profile" />}
            ...
            <h1 className="text-xl font-semibold">{props.user?.name}</h1>
          </div>
        ...
        <div>
          <h2 className="text-lg font-semibold">About {props.user?.name}</h2>
          <p className="text-gray-500">{props.user?.about}</p>
        </div>
      </div>

      <div className="bg-white border border-[#E4E4E7] rounded-lg p-6">
        <h2 className="text-lg font-semibold">{props.user?.socialMediaURL}</h2>
        <a href={props.user?.socialMediaURL} target="_blank" className="text-blue-500">
          {props.user?.socialMediaURL}
        </a>
      </div>
