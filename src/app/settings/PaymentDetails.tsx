...
export default function PaymentDetails() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/bank-card/${localId}`);
  ...
  return (
    <div className="w-[650px] min-h-[250px] text-black gap-1 p-[24px] flex flex-col rounded-[9px] border-[#E4E4E7] border-[1px] ">
      ...
      <div className="flex justify-between">
        ...
        <h2 className="text-[14px] font-semibold flex flex-col">
          CVC
          <input className="rounded-[6px] border-[#E4E4E7] border-[1px] p-2 w-[192px]" type="number" placeholder="XXX" />
        </h2>
      </div>
